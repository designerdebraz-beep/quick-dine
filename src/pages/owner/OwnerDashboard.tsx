...
import { getMyRestaurants } from "../../lib/restaurants.js";
import { getRestaurantBookings } from "../../lib/bookings.js";
...
export default function OwnerDashboard() {
    ...
    const fetchOwnerData = async () => {
        try {
            const owned = await getMyRestaurants(token as string);
            const rest = owned[0] || null;
            setRestaurant(rest);
            if (rest) {
                const bks = await getRestaurantBookings(rest._id, token as string);
                setBookings(bks);
            } else {
                setBookings([]);
            }
