...
import { getAdminRestaurants, getAdminStats, updateRestaurantStatus } from "../../lib/admin.js";
...
export default function AdminDashboard() {
    ...
    const fetchAdminData = async () => {
        try {
            const [restaurants, stats] = await Promise.all([
                getAdminRestaurants(token as string),
                getAdminStats(token as string),
            ]);
    ...
    const handleApproveStatus = async (restaurantId: string, status: "approved" | "rejected") => {
        try {
            ...
            const updated = await updateRestaurantStatus(restaurantId, status, token as string);
