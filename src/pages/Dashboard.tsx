...
import { getMyBookings, updateBookingStatus } from "../lib/bookings.js";
...
export default function Dashboard() {
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                const data = await getMyBookings(token as string);
                setBookings(data);
            } catch (error) {
                console.error(error);
                setBookings([]);
    ...
    const handleCancelBooking = async (bookingId: string) => {
        ...
        try {
            await updateBookingStatus(bookingId, "cancelled", token as string);
